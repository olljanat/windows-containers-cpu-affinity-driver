...
## How it works?
Windows will start new `wininit.exe` (Container Execution Agent) process for every container and all processes starting inside of container will be child of that.

`ContainerCpuAffinity` driver will detect `wininit.exe` when starts and set CPU affinity for it in way that it can use all other CPU cores expect two first ones.
